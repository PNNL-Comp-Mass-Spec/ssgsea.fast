...
time_df <- lapply(seq_len(1L), function(j) { # 1 replicate due to long runtimes
   ...
   for (i in seq_len(nrow(comb_df))) {
      ...
      write(x = elapsed_time,
            file = "simulation/data/ssGSEA-base_elapsed_time.txt",
            append = TRUE,
            sep = "\n")
...
saveRDS(
   ...
   file = file.path(
      "simulation",
      "data",
      "ssGSEA-base_timing_results_for_comparison.rds"
   ),
   compress = TRUE,
   version = 3L
)
...
# No longer needed
file.remove("simulation/data/ssGSEA-base_elapsed_time.txt")
