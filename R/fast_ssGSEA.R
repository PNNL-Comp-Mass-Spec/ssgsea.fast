...
#'   allowed.
#' @param gene_sets a named list of molecular signatures to test. Each element of
#'   the list must be a character vector. Most commonly, molecular signatures
...
#'   significantly increase runtime but decrease memory consumption, so this
                        ...
                        seed = NULL)
{
   ...
   ES_list <- .calcES(alpha = alpha,
                      Y_prime = Y[, rownames(A), drop = FALSE],
                      R_prime = R[, rownames(A), drop = FALSE],
                      sumRanks = sumRanks,
                      A = A,
                      M = M,
                      W = W,
