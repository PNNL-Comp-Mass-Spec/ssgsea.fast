                                 ...
                                 m_d_i = NULL)
{
   ...
   if (directional) {
      ...
      # Numer of genes expected to be up-regulated in each set. May not be
      # unique.
      ...
      max_set_size <- max(unique_set_sizes) # largest total set size
      ...
      theta_m_i <- unique_set_sizes <- unique(m_i)
   ...
   ls <- lapply(unique_set_sizes, seq_len)

   # The first unique_set_sizes[i] entries of the i-th row are all 1. The
   # remaining entries are 0.
   A_perm <- rep(0, length(unique_set_sizes) * max_set_size)
   dim(A_perm) <- c(length(unique_set_sizes), max_set_size)

   A_perm[cbind(rep(seq_along(ls), unique_set_sizes),
                unlist(ls))] <- 1

   if (directional) {
      ls_d <- lapply(theta_m_i, seq_len)

      A_perm_d <- A_perm

      # Replace the first \theta(m_i) entries in each row i with 0
      A_perm_d[cbind(rep(seq_along(ls_d), lengths(ls_d)),
                     unlist(ls_d))] <- 0

      # A_perm and A_perm_d should be disjoint
...
#'
#' @param ES numeric vector of enrichment scores (ES).
...
#' @param rep_idx integer vector that maps each entry of \code{ES} to a row of
#'   \code{ES_perm}.
#'
...
#' @author Tyler Sagendorf
#'
#' @importFrom data.table data.table := setorderv rbindlist setDT
#'
#' @noRd
.extractPermInfo <- function(ES,
                             ES_perm,
                             rep_idx)
{
   dt_ES <- data.table(ES = ES,
                       ES_idx = seq_along(ES),
                       rep_idx = rep_idx,
                       stringsAsFactors = FALSE)

   dt_rep_idx <- dt_ES[, c("ES_idx", "rep_idx")]
   ...
   dt_ES <- subset(dt_ES, subset = !is.na(ES))

   dt_ES[, rep_idx := factor(rep_idx, levels = seq_len(nrow(ES_perm)))]

   ES_ls <- split(x = dt_ES[["ES"]],
                  f = dt_ES[["rep_idx"]])
   ...
   out <- lapply(seq_along(ES_ls), function(i) {
      ...
      out_i <- .Rcpp_extractPermInfo(ES_i, ES_perm_i)
   ...
   # It is very important that dt_ES is sorted by repIdx first, since this
   # determines the chunks of results in the out data.table.
   out[, ES_idx := dt_ES[["ES_idx"]]]

   # This will add rows for ES that were NA and removed earlier
   out <- merge(x = out,
                y = dt_rep_idx,
                by = "ES_idx",
                all = TRUE)
   ...
   # Original order
   setorderv(out, cols = "ES_idx", order = 1L)
   out[, ES_idx := NULL]
...
#'   "set_size", "direction", "ES_u" (optional), "ES_d" (optional), "ES", "NES",
...
#'
#'   \item{direction}{factor; direction of change ("Up" or "Down").}
#'
...
#' @author Tyler Sagendorf
#'
#' @importFrom data.table rbindlist := setcolorder setorderv
                          ...
                          adjust_globally = FALSE)
{
   ...
   tab[, `:=`(sample = factor(sample, levels = unique(sample)),
              set_size = as.integer(set_size),
              direction = factor(ES >= 0,
                                 levels = c(TRUE, FALSE),
                                 labels = c("Up", "Down")),
              p_value = (n_as_extreme + 1L) / (n_same_sign + 1L))]

   # Requires data.table >= 1.15.0
   setcolorder(tab,
               neworder = "direction",
               after = "set_size")
...
.makeResultsTable <- function(alpha = 1,
                              ...
                              ES_d_i)
{
   # Initialize vectors of 0's. These vectors will be updated using the
   # results from each batch of permutations.
   n_same_sign <- n_as_extreme <- rep(0L, length(ES_i))
   ...
   if (nperm != 0L) {
      ...
      for (b in seq_along(seed_list)) {
         ...
         perm_dt <- .extractPermInfo(ES = ES_i,
                                     ES_perm = ES_perm,
                                     rep_idx = rep_idx)
   ...
   # Mean of permutation ES used to normalize the true ES
   mean_ES_perm <- sum_ES_perm / n_same_sign
   mean_ES_perm[n_same_sign == 0L] <- NA

   NES <- ES_i / mean_ES_perm
